...
export default function CashReceivedPage() {
  ...
  const onAreaChange = async (areaId: string) => {
    ...
    try {
      ...
      setPersonsInArea(p || []);
      // load previous debit notes for this area
  ...
  const addRecord = async () => {
    ...
    try {
      ...
      // clear amount and month for next entry
      setAmount('');
      setSelectedMonth('');
    } catch (e: any) {
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-4 items-end">
          <div className="md:col-span-1">
            ...
            >
              ...
              {areas.map((a) => (
                <option key={a._id} value={a._id}>{a.connectionNumber ? `${a.connectionNumber} — ${a.name}` : a.name}</option>
              ))}
            </select>
          </div>
          ...
          <div>
            ...
            >
              ...
              {personsInArea.map((p) => (
                <option key={p._id} value={p._id}>{p.number ?? p.connectionNumber ?? p.name}</option>
              ))}
            </select>
          </div>
